	...
	<section id="course_list">
		...
		<div class="courses_container courses_body">
			...
			<main class="courses_main">
				...
				<!-- courseList start -->
				<div id="listsC" class="courses_card_list_body">
				...
				<c:if test="${paging.pages != 1 }">
					<c:choose>
						<c:when test="${paging.pages < 6 }">
							<div class="paging-wrapper">
								<ul class="pagination mx-auto" style="width: max-content;">
									<c:forEach begin="1" end="${paging.pages }" step="1" var="i">
										<li class="page-item"><a class="page-link" href="${path }/courses?paging=${paging}&now=${i}">${i}</a></li>
									</c:forEach>
								</ul>
							</div>
						</c:when>
...
<script type="text/javascript">
	...
	function moreViewAjax(){
		let begin = $("#listsC").children().length+1;
		let end = begin + 4;
		let map = { "begin" : begin , "end" : end};
		alert(begin);
		$.ajax({
			url: "${path}/course/moreCourse",
			type: "post",
			dataType: "json",
			data: map,
			success: function(data){
				$.each(data,function(){
					...
					//$("#listsC").append($("<div>"+item.course_seq+"</div>"));
				});
			}
		});
	}
