...
public class MemberController {
	...
	// 비밀번호 찾기
	@GetMapping("/findPw")
	public String showFindPwJsp() {
		return "member/findPw";
	}
	
	@ResponseBody
	@PostMapping("/createSamples")
	public Boolean createSamples() {
		log.info("예제 회원 데이터 생성 >>>>>>>>>>>> ");
		return memberService.insertSampleMembers();
	}
	
	@ResponseBody
	@PostMapping("/checkEmail")
	public Map<String,Object> checkEmail(MemberVO member,HttpServletResponse response) {
		log.info("비밀번호 변경 접근 >>>>>>>>>>>> " + member);
		return memberService.checkEmailForChangePw(member,response);
	}
}
