...
public class OrderServiceImpl implements OrderService {
	...
	/* 멘토링 */
	@Override
	   public MenteeVO payMenteeList(int mentee_seq) {
	      log.info("order serviceImpl 들어옴>>>>>>>>>>>>>>>>");
	      MenteeVO payMenteeList = mentoringMapper.selectPayMyMenteeBySeq(mentee_seq);
	      log.info("payMenteeList>>>>>>>>>>return>>>>>>" + payMenteeList);
	      return payMenteeList;
	}
	
	@Override
	public Map<String, Object> purchasePage(OrderVO orderVO) {
		log.info("order serviceImpl 결제imp들어옴>>>>>>>>>>>>>>>>");
		Map<String, Object> map = new HashMap<String, Object>();
			 //이제 구매
			 int result = orderMapper.insertBuyMentroing(orderVO);
			 log.info("result 멘토링 구매 구매 구매>>>>>>구매완료>>>>>>>>>>" + result);
			 map.put("result", true);
			 map.put("msg", "구매가 완료되었습니다. <br> 마이 페이지로 이동합니다.");
	...
	//구매기록
	@Override
	public List<OrderVO> purchaseHistory(String member_id) {
		log.info("구매했던 기록보러 들어옴>>>>>>>>>>>>>>");
		List<OrderVO> purchaseHistory = orderMapper.selectSaleHistoryAll(member_id);
		log.info(" 기록들고 나감>>>>>>>>>>>>>>" + purchaseHistory);
		return purchaseHistory.size() == 0 ? null : purchaseHistory;
	}
	//판매 기록
	@Override
	public List<OrderVO> salesHistory(String member_id) {
		log.info("판매했던 기록보러 들어옴>>>>>>>>>>>>>>");
		List<OrderVO> salesHistory = orderMapper.selectBuyHistoryAll(member_id);
		log.info(" 기록들고 나감>>>>>>>>>>>>>>" + salesHistory);
		return salesHistory.size() == 0 ? null : salesHistory;
	}


	@Override
	public List<OrderVO> orderList(OrderVO orderVO) {
		log.info("관리자:결제 기록보러 들어옴>>>>>>>>>>>>>>");
		List<OrderVO> orderList = orderMapper.selectAllPayList(orderVO);
		log.info("관리자:결제 기록들고 나감>>>>>>>>>>>>>>" + orderList);
