...
public class OrderController {
	...
	@GetMapping("/purchaseMentoring/{mentee_seq}")
	   @LoginRequired
	   public String orderPage(@PathVariable int mentee_seq, HttpSession session, Model model) {
	      UserVO user = (UserVO) session.getAttribute("user");
	      MenteeVO menteeList = orderService.payMenteeList(mentee_seq);
	      log.info("order Page controller payMentee 구매자 정보입니다." + menteeList);
	      model.addAttribute("menteeList", menteeList);
	      return "/order/orderMentoring";
	}

	/* 멘토링 */
	@PostMapping("/purchaseMentoring")
	@LoginRequired
	@ResponseBody
	public Map<String, Object> purchasePage(OrderVO orderVO, HttpSession session) {
		UserVO user = (UserVO) session.getAttribute("user");
		Map<String, Object> map = orderService.purchasePage(orderVO);
		return map;
	}
}
